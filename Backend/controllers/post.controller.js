...
export const getUserPost = async (req, res) => {
    try {
        ...
        const posts = await Post.find({ author: authorId }).sort({ createdAt: -1 }).populate({
            ...
            path: 'comments', options: { sort: { createdAt: -1 } }, populate: { path: 'author', select: 'username profilePicture' }
        });
        return res.status(200).json({ posts, success: true });
    } catch (error) {
        console.log(error);
...
export const likePost = async (req, res) => {
    try {
        ...
        if (postOwnerId !== likeKrneWalaUserKiId) {
            const notification = { type: 'like', userId: likeKrneWalaUserKiId, userDetails: user, postId, message: 'Your post was liked' };
...
export const dislikePost = async (req, res) => {
    try {
        ...
        const postOwnerId = post.author.toString();
        if (postOwnerId !== likeKrneWalaUserKiId) {
            const notification = { type: 'dislike', userId: likeKrneWalaUserKiId, userDetails: user, postId, message: 'Your post was disliked' };
            const postOwnerSocketId = getReceiverSocketId(postOwnerId);
            if (postOwnerSocketId) io.to(postOwnerSocketId).emit('notification', notification);
        }
